...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<script>alert('$user, bem vindo de volta!'); window.location.href='home.php';</script>";
        header("Location: home.php");
            exit();
        } else {
            // Senha incorreta
        echo "<script>alert('Senha incorreta.'); window.location.href='entre.php';</script>";
            exit();
        }
    } else {
        // E-mail não encontrado
        echo "<script>alert('Email não encontrado'); window.location.href='entre.php';</script>";
        exit();
    }
